...
func NewNatsConnection(conf config.NatsConfig, logger *zap.Logger) (NatsConnection, error) {
	logger.With(zap.String("nats connection", config.GetNatsURL(conf.Host, conf.ClientPort)))

	nc, err := nats.Connect(
		config.GetNatsURL(conf.Host, conf.ClientPort),
	...
	if err != nil {
		logger.Error("Failed to connect to nats",
			zap.Error(err),
			zap.String("nats connection", config.GetNatsURL(conf.Host, conf.ClientPort)),
		)
		return nil, err
	}
	...
	if streamInfo != nil {
		...
		streamCfg := nats.StreamConfig{
			Name:         streamName,
			Subjects:     conf.StreamConfig.Subjects,
			Retention:    retention,
			Discard:      discard,
			Storage:      storage,
			NoAck:        conf.StreamConfig.NoAck,
			MaxConsumers: conf.StreamConfig.MaxConsumers,
			MaxBytes:     conf.StreamConfig.MaxBytes,
		}
